...
namespace Repository.Repositories
{
    public class LibraryRepository : ILibraryRepository
    {
        ...
        public void IngresoInformacion(decimal tarifaBasica, string temporada, string aerolinea, int edad, bool esEstudiante0)
        {
            decimal valorAcobrar = 0;

            valorAcobrar = procesarAerolinea(tarifaBasica, aerolinea, temporada);
        }

        public decimal procesarAerolinea(decimal tarifaBasica, string aerolinea, string temporada)
        {
            decimal incrementoAerolinea = 0;

            if (aerolinea == "ALAS" && temporada == "ALTA")
            {
                incrementoAerolinea = tarifaBasica * (30 / 100);
            } 
            else if (aerolinea == "" && temporada == "ALTA")
            {
                incrementoAerolinea = tarifaBasica * (20 / 100);
            };

            return incrementoAerolinea;
        }
    }
}
